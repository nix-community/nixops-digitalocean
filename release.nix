{ src ? {
  ...
  shortRev = "abcdef";
  rev = "HEAD";
}, officialRelease ? false, nixpkgs ? <nixpkgs> }:
let
  pkgs = import nixpkgs { system = "x86_64-linux"; };
  version = "1.7" + (if officialRelease then
    ""
  else if src ? lastModified then
    "pre${builtins.substring 0 8 src.lastModified}.${src.shortRev}"
  else
    "pre${toString (src.revCount or 0)}_${src.shortRev or "abcdef"}");
in rec {
  build = pkgs.lib.genAttrs [ "x86_64-linux" "i686-linux" "x86_64-darwin" ]
    (system:
      ...
      python2Packages.buildPythonApplication rec {
        ...
        propagatedBuildInputs = with python2Packages; [ digital-ocean ];
        ...
        makeWrapperArgs =
          [ "--prefix" "PATH" ":" "${openssh}/bin" "--set" "PYTHONPATH" ":" ];
